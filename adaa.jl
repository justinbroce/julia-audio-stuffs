...
function test()
    ...
    seed = (getSaw(1,"f6").+getSaw(1,"f7"))/2
    seed = applyEnvelope(seed, Envelope(41000,.3,.3,.5,.2,.1))

   
   
    @time k = sc.(5seed) 
    @time e = processAD1(5seed, sc,sc1) 
    @time g = processAD2(5seed, sc, sc1, sc2) 
    p = hardC(seed,5)
    @time c = processAD1(5seed, f,f1)
    @time b = processAD2(5seed, f,f1,f2)
    ...
    append!(seed,b)
    wavplay(seed,41000)
    wavwrite(seed, 41000, "aada test.wav")
end
test()