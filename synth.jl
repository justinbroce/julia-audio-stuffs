...
function plotWave(wave , title_ =  "Wave", fs = 41000.0, xlim_ = 800)
    plot(getTimeVector(1, fs), wave, xaxis = "time (seconds)", yaxis = "amplitude", label=nothing,
    title=title_, titlefont=10, x_lims = (1/fs,xlim_/fs))
end    
...
function disco()
    ...
    wavwrite(rendered1,41000, "demo.wav")
